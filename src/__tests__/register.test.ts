...
describe('POST /api/register', () => {
  describe('success', () => {
    it('returns 204 on valid request with a single student', async () => {
      ...
      expect(mockRegisterStudents).toHaveBeenCalledWith('teacherken@gmail.com', ['studentjon@gmail.com']);
    });
  ...
  describe('validation errors', () => {
    it('returns 400 when teacher field is missing', async () => {
      const res = await request(app).post('/api/register').send({ students: ['studentjon@gmail.com'] });
